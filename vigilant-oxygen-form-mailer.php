...
function vigilant_oxygen_form_mailer_block_legacy_form_mail($return, $atts)
{
    if (!empty($GLOBALS['vigilant_oxygen_form_mailer_allow_admin_wp_mail'])) {
        return $return;
    }

    $subject = strtolower(trim(sanitize_text_field($atts['subject'] ?? '')));

    if ($subject === 'new contact form message') {
...
add_filter('pre_wp_mail', 'vigilant_oxygen_form_mailer_block_legacy_form_mail', 20, 2);
...
function vigilant_oxygen_form_mailer_send_admin_notification($submission)
{
    ...
    $GLOBALS['vigilant_oxygen_form_mailer_allow_admin_wp_mail'] = true;
    $sent = wp_mail(
        $to_email,
    ...
    unset($GLOBALS['vigilant_oxygen_form_mailer_allow_admin_wp_mail']);
